...
/^<(\w+)\s*\/?>(?:<\/\1>)?$/,E=/^[\],:{}\s]*$/,y=/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,H=/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,D=/(?:^|:|,)(?:\s*\[)+/g,S=/(webkit)[ \/]([\w.]+)/,$=/(opera)(?:.*version)?[ \/]([\w.]+)/,ba=/(msie) ([\w.]+)/,aa=/(mozilla)(?:.*? rv:([\w.]+))?/,na=/-([a-z]|[0-9])/ig,T=/^-ms-/,ma=function(o,A){return(A+"").toUpperCase()},ia=Qb.userAgent,i,k,u=Object.prototype.toString,x=Object.prototype.hasOwnProperty,z=Array.prototype.push,B=Array.prototype.slice,
...
null?String(o):V[u.call(o)]||"object"},isPlainObject:function(o){if(!o||b.type(o)!=="object"||o.nodeType||b.isWindow(o))return false;try{if(o.constructor&&!x.call(o,"constructor")&&!x.call(o.constructor.prototype,"isPrototypeOf"))return false}catch(A){return false}var K;for(K in o);return K===j||x.call(o,K)},isEmptyObject:function(o){for(var A in o)return false;return true},error:function(o){throw new Error(o);},parseJSON:function(o){if(typeof o!=="string"||!o)return null;o=b.trim(o);if(g.JSON&&g.JSON.parse)return g.JSON.parse(o);
...
c.event.dispatch;c.removeEvent=L.removeEventListener?function(a,b,d){a.removeEventListener&&a.removeEventListener(b,d,false)}:function(a,b,d){a.detachEvent&&a.detachEvent("on"+b,d)};c.Event=function(a,b){if(!(this instanceof c.Event))return new c.Event(a,b);if(a&&a.type){this.originalEvent=a;this.type=a.type;this.isDefaultPrevented=a.defaultPrevented||a.returnValue===false||a.getPreventDefault&&a.getPreventDefault()?w:r}else this.type=a;b&&c.extend(this,b);this.timeStamp=a&&a.timeStamp||c.now();this[c.expando]=
true};c.Event.prototype={preventDefault:function(){this.isDefaultPrevented=w;var a=this.originalEvent;if(a)if(a.preventDefault)a.preventDefault();else a.returnValue=false},stopPropagation:function(){this.isPropagationStopped=w;var a=this.originalEvent;if(a){a.stopPropagation&&a.stopPropagation();a.cancelBubble=true}},stopImmediatePropagation:function(){this.isImmediatePropagationStopped=w;this.stopPropagation()},isDefaultPrevented:r,isPropagationStopped:r,isImmediatePropagationStopped:r};c.each({mouseenter:"mouseover",
...
0?this.on(b,null,d,e):this.trigger(b)};if(c.attrFn)c.attrFn[b]=true;if(Xb.test(b))c.event.fixHooks[b]=c.event.keyHooks;if(Yb.test(b))c.event.fixHooks[b]=c.event.mouseHooks});(function(){function a(i,k,u,x,z,B){z=0;for(var J=x.length;z<J;z++){var M=x[z];if(M){var V=false;for(M=M[i];M;){if(M[e]===u){V=x[M.sizset];break}if(M.nodeType===1&&!B){M[e]=u;M.sizset=z}if(M.nodeName.toLowerCase()===k){V=M;break}M=M[i]}x[z]=V}}}function b(i,k,u,x,z,B){z=0;for(var J=x.length;z<J;z++){var M=x[z];if(M){var V=false;
for(M=M[i];M;){if(M[e]===u){V=x[M.sizset];break}if(M.nodeType===1){if(!B){M[e]=u;M.sizset=z}if(typeof k!=="string"){if(M===k){V=true;break}}else if(y.filter(k,[M]).length>0){V=M;break}}M=M[i]}x[z]=V}}}var d=/((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g,e="sizcache"+(Math.random()+"").replace(".",""),f=0,h=Object.prototype.toString,l=false,m=true,n=/\\/g,t=/\r\n/g,E=/\W/;[0,0].sort(function(){m=false;return 0});
var y=function(i,k,u,x){u=u||[];var z=k=k||L;if(k.nodeType!==1&&k.nodeType!==9)return[];if(!i||typeof i!=="string")return u;var B,J,M,V,o,A=true,K=y.isXML(k),F=[],X=i;do{d.exec("");if(B=d.exec(X)){X=B[3];F.push(B[1]);if(B[2]){V=B[3];break}}}while(B);if(F.length>1&&S.exec(i))if(F.length===2&&D.relative[F[0]])J=ia(F[0]+F[1],k,x);else for(J=D.relative[F[0]]?[k]:y(F.shift(),k);F.length;){i=F.shift();if(D.relative[i])i+=F.shift();J=ia(i,J,x)}else{if(!x&&F.length>1&&k.nodeType===9&&!K&&D.match.ID.test(F[0])&&
!D.match.ID.test(F[F.length-1])){B=y.find(F.shift(),k,K);k=B.expr?y.filter(B.expr,B.set)[0]:B.set[0]}if(k){B=x?{expr:F.pop(),set:aa(x)}:y.find(F.pop(),F.length===1&&(F[0]==="~"||F[0]==="+")&&k.parentNode?k.parentNode:k,K);J=B.expr?y.filter(B.expr,B.set):B.set;if(F.length>0)M=aa(J);else A=false;for(;F.length;){B=o=F.pop();if(D.relative[o])B=F.pop();else o="";if(B==null)B=k;D.relative[o](M,B,K)}}else M=[]}M||(M=J);M||y.error(o||i);if(h.call(M)==="[object Array]")if(A)if(k&&k.nodeType===1)for(i=0;M[i]!=
null;i++){if(M[i]&&(M[i]===true||M[i].nodeType===1&&y.contains(k,M[i])))u.push(J[i])}else for(i=0;M[i]!=null;i++)M[i]&&M[i].nodeType===1&&u.push(J[i]);else u.push.apply(u,M);else aa(M,u);if(V){y(V,z,u,x);y.uniqueSort(u)}return u};y.uniqueSort=function(i){if(T){l=m;i.sort(T);if(l)for(var k=1;k<i.length;k++)i[k]===i[k-1]&&i.splice(k--,1)}return i};y.matches=function(i,k){return y(i,null,null,k)};y.matchesSelector=function(i,k){return y(k,null,null,[i]).length>0};y.find=function(i,k,u){var x,z,B,J,M,
V;if(!i)return[];z=0;for(B=D.order.length;z<B;z++){M=D.order[z];if(J=D.leftMatch[M].exec(i)){V=J[1];J.splice(1,1);if(V.substr(V.length-1)!=="\\"){J[1]=(J[1]||"").replace(n,"");x=D.find[M](J,k,u);if(x!=null){i=i.replace(D.match[M],"");break}}}}x||(x=typeof k.getElementsByTagName!=="undefined"?k.getElementsByTagName("*"):[]);return{set:x,expr:i}};y.filter=function(i,k,u,x){for(var z,B,J,M,V,o,A,K,F=i,X=[],U=k,ca=k&&k[0]&&y.isXML(k[0]);i&&k.length;){for(J in D.filter)if((z=D.leftMatch[J].exec(i))!=null&&
z[2]){o=D.filter[J];V=z[1];B=false;z.splice(1,1);if(V.substr(V.length-1)!=="\\"){if(U===X)X=[];if(D.preFilter[J])if(z=D.preFilter[J](z,U,u,X,x,ca)){if(z===true)continue}else B=M=true;if(z)for(A=0;(V=U[A])!=null;A++)if(V){M=o(V,z,A,U);K=x^M;if(u&&M!=null)if(K)B=true;else U[A]=false;else if(K){X.push(V);B=true}}if(M!==j){u||(U=X);i=i.replace(D.match[J],"");if(!B)return[];break}}}if(i===F)if(B==null)y.error(i);else break;F=i}return U};y.error=function(i){throw new Error("Syntax error, unrecognized expression: "+
i);};var H=y.getText=function(i){var k,u;k=i.nodeType;var x="";if(k)if(k===1||k===9)if(typeof i.textContent==="string")return i.textContent;else if(typeof i.innerText==="string")return i.innerText.replace(t,"");else for(i=i.firstChild;i;i=i.nextSibling)x+=H(i);else{if(k===3||k===4)return i.nodeValue}else for(k=0;u=i[k];k++)if(u.nodeType!==8)x+=H(u);return x},D=y.selectors={order:["ID","NAME","TAG"],match:{ID:/#((?:[\w\u00c0-\uFFFF\-]|\\.)+)/,CLASS:/\.((?:[\w\u00c0-\uFFFF\-]|\\.)+)/,NAME:/\[name=['"]*((?:[\w\u00c0-\uFFFF\-]|\\.)+)['"]*\]/,
...
type:function(i){return i.getAttribute("type")}},relative:{"+":function(i,k){var u=typeof k==="string",x=u&&!E.test(k);u=u&&!x;if(x)k=k.toLowerCase();x=0;for(var z=i.length,B;x<z;x++)if(B=i[x]){for(;(B=B.previousSibling)&&B.nodeType!==1;);i[x]=u||B&&B.nodeName.toLowerCase()===k?B||false:B===k}u&&y.filter(k,i,true)},">":function(i,k){var u,x=typeof k==="string",z=0,B=i.length;if(x&&!E.test(k))for(k=k.toLowerCase();z<B;z++){if(u=i[z]){u=u.parentNode;i[z]=u.nodeName.toLowerCase()===k?u:false}}else{for(;z<
B;z++)if(u=i[z])i[z]=x?u.parentNode:u.parentNode===k;x&&y.filter(k,i,true)}},"":function(i,k,u){var x,z=f++,B=b;if(typeof k==="string"&&!E.test(k)){x=k=k.toLowerCase();B=a}B("parentNode",k,z,i,x,u)},"~":function(i,k,u){var x,z=f++,B=b;if(typeof k==="string"&&!E.test(k)){x=k=k.toLowerCase();B=a}B("previousSibling",k,z,i,x,u)}},find:{ID:function(i,k,u){if(typeof k.getElementById!=="undefined"&&!u)return(i=k.getElementById(i[1]))&&i.parentNode?[i]:[]},NAME:function(i,k){if(typeof k.getElementsByName!==
"undefined"){var u=[];k=k.getElementsByName(i[1]);for(var x=0,z=k.length;x<z;x++)k[x].getAttribute("name")===i[1]&&u.push(k[x]);return u.length===0?null:u}},TAG:function(i,k){if(typeof k.getElementsByTagName!=="undefined")return k.getElementsByTagName(i[1])}},preFilter:{CLASS:function(i,k,u,x,z,B){i=" "+i[1].replace(n,"")+" ";if(B)return i;B=0;for(var J;(J=k[B])!=null;B++)if(J)if(z^(J.className&&(" "+J.className+" ").replace(/[\t\n\r]/g," ").indexOf(i)>=0))u||x.push(J);else if(u)k[B]=false;return false},
ID:function(i){return i[1].replace(n,"")},TAG:function(i){return i[1].replace(n,"").toLowerCase()},CHILD:function(i){if(i[1]==="nth"){i[2]||y.error(i[0]);i[2]=i[2].replace(/^\+|\s*/g,"");var k=/(-?)(\d*)(?:n([+\-]?\d*))?/.exec(i[2]==="even"&&"2n"||i[2]==="odd"&&"2n+1"||!/\D/.test(i[2])&&"0n+"+i[2]||i[2]);i[2]=k[1]+(k[2]||1)-0;i[3]=k[3]-0}else i[2]&&y.error(i[0]);i[0]=f++;return i},ATTR:function(i,k,u,x,z,B){k=i[1]=i[1].replace(n,"");if(!B&&D.attrMap[k])i[1]=D.attrMap[k];i[4]=(i[4]||i[5]||"").replace(n,
"");if(i[2]==="~=")i[4]=" "+i[4]+" ";return i},PSEUDO:function(i,k,u,x,z){if(i[1]==="not")if((d.exec(i[3])||"").length>1||/^\w/.test(i[3]))i[3]=y(i[3],null,null,k);else{i=y.filter(i[3],k,u,true^z);u||x.push.apply(x,i);return false}else if(D.match.POS.test(i[0])||D.match.CHILD.test(i[0]))return true;return i},POS:function(i){i.unshift(true);return i}},filters:{enabled:function(i){return i.disabled===false&&i.type!=="hidden"},disabled:function(i){return i.disabled===true},checked:function(i){return i.checked===
...
return k==="input"&&"button"===i.type||k==="button"},input:function(i){return/input|select|textarea|button/i.test(i.nodeName)},focus:function(i){return i===i.ownerDocument.activeElement}},setFilters:{first:function(i,k){return k===0},last:function(i,k,u,x){return k===x.length-1},even:function(i,k){return k%2===0},odd:function(i,k){return k%2===1},lt:function(i,k,u){return k<u[3]-0},gt:function(i,k,u){return k>u[3]-0},nth:function(i,k,u){return u[3]-0===k},eq:function(i,k,u){return u[3]-0===k}},filter:{PSEUDO:function(i,
k,u,x){var z=k[1],B=D.filters[z];if(B)return B(i,u,k,x);else if(z==="contains")return(i.textContent||i.innerText||H([i])||"").indexOf(k[3])>=0;else if(z==="not"){k=k[3];u=0;for(x=k.length;u<x;u++)if(k[u]===i)return false;return true}else y.error(z)},CHILD:function(i,k){var u,x,z,B;u=k[1];var J=i;switch(u){case "only":case "first":for(;J=J.previousSibling;)if(J.nodeType===1)return false;if(u==="first")return true;J=i;case "last":for(;J=J.nextSibling;)if(J.nodeType===1)return false;return true;case "nth":u=
k[2];x=k[3];if(u===1&&x===0)return true;k=k[0];if((z=i.parentNode)&&(z[e]!==k||!i.nodeIndex)){B=0;for(J=z.firstChild;J;J=J.nextSibling)if(J.nodeType===1)J.nodeIndex=++B;z[e]=k}i=i.nodeIndex-x;return u===0?i===0:i%u===0&&i/u>=0}},ID:function(i,k){return i.nodeType===1&&i.getAttribute("id")===k},TAG:function(i,k){return k==="*"&&i.nodeType===1||!!i.nodeName&&i.nodeName.toLowerCase()===k},CLASS:function(i,k){return(" "+(i.className||i.getAttribute("class"))+" ").indexOf(k)>-1},ATTR:function(i,k){var u=
k[1];i=y.attr?y.attr(i,u):D.attrHandle[u]?D.attrHandle[u](i):i[u]!=null?i[u]:i.getAttribute(u);u=i+"";var x=k[2];k=k[4];return i==null?x==="!=":!x&&y.attr?i!=null:x==="="?u===k:x==="*="?u.indexOf(k)>=0:x==="~="?(" "+u+" ").indexOf(k)>=0:!k?u&&i!==false:x==="!="?u!==k:x==="^="?u.indexOf(k)===0:x==="$="?u.substr(u.length-k.length)===k:x==="|="?u===k||u.substr(0,k.length+1)===k+"-":false},POS:function(i,k,u,x){var z=D.setFilters[k[2]];if(z)return z(i,u,k,x)}}},S=D.match.POS,$=function(i,k){return"\\"+
(k-0+1)};for(var ba in D.match){D.match[ba]=new RegExp(D.match[ba].source+/(?![^\[]*\])(?![^\(]*\))/.source);D.leftMatch[ba]=new RegExp(/(^(?:.|\r|\n)*?)/.source+D.match[ba].source.replace(/\\(\d+)/g,$))}var aa=function(i,k){i=Array.prototype.slice.call(i,0);if(k){k.push.apply(k,i);return k}return i};try{Array.prototype.slice.call(L.documentElement.childNodes,0)}catch(na){aa=function(i,k){var u=0;k=k||[];if(h.call(i)==="[object Array]")Array.prototype.push.apply(k,i);else if(typeof i.length==="number")for(var x=
i.length;u<x;u++)k.push(i[u]);else for(;i[u];u++)k.push(i[u]);return k}}var T,ma;if(L.documentElement.compareDocumentPosition)T=function(i,k){if(i===k){l=true;return 0}if(!i.compareDocumentPosition||!k.compareDocumentPosition)return i.compareDocumentPosition?-1:1;return i.compareDocumentPosition(k)&4?-1:1};else{T=function(i,k){if(i===k){l=true;return 0}else if(i.sourceIndex&&k.sourceIndex)return i.sourceIndex-k.sourceIndex;var u,x,z=[],B=[];u=i.parentNode;x=k.parentNode;var J=u;if(u===x)return ma(i,
k);else if(u){if(!x)return 1}else return-1;for(;J;){z.unshift(J);J=J.parentNode}for(J=x;J;){B.unshift(J);J=J.parentNode}u=z.length;x=B.length;for(J=0;J<u&&J<x;J++)if(z[J]!==B[J])return ma(z[J],B[J]);return J===u?ma(i,B[J],-1):ma(z[J],k,1)};ma=function(i,k,u){if(i===k)return u;for(i=i.nextSibling;i;){if(i===k)return-1;i=i.nextSibling}return 1}}(function(){var i=L.createElement("div"),k="script"+(new Date).getTime(),u=L.documentElement;i.innerHTML="<a name='"+k+"'/>";u.insertBefore(i,u.firstChild);
if(L.getElementById(k)){D.find.ID=function(x,z,B){if(typeof z.getElementById!=="undefined"&&!B)return(z=z.getElementById(x[1]))?z.id===x[1]||typeof z.getAttributeNode!=="undefined"&&z.getAttributeNode("id").nodeValue===x[1]?[z]:j:[]};D.filter.ID=function(x,z){var B=typeof x.getAttributeNode!=="undefined"&&x.getAttributeNode("id");return x.nodeType===1&&B&&B.nodeValue===z}}u.removeChild(i);u=i=null})();(function(){var i=L.createElement("div");i.appendChild(L.createComment(""));if(i.getElementsByTagName("*").length>
0)D.find.TAG=function(k,u){u=u.getElementsByTagName(k[1]);if(k[1]==="*"){k=[];for(var x=0;u[x];x++)u[x].nodeType===1&&k.push(u[x]);u=k}return u};i.innerHTML="<a href='#'></a>";if(i.firstChild&&typeof i.firstChild.getAttribute!=="undefined"&&i.firstChild.getAttribute("href")!=="#")D.attrHandle.href=function(k){return k.getAttribute("href",2)};i=null})();L.querySelectorAll&&function(){var i=y,k=L.createElement("div");k.innerHTML="<p class='TEST'></p>";if(!(k.querySelectorAll&&k.querySelectorAll(".TEST").length===
0)){y=function(x,z,B,J){z=z||L;if(!J&&!y.isXML(z)){var M=/^(\w+$)|^\.([\w\-]+$)|^#([\w\-]+$)/.exec(x);if(M&&(z.nodeType===1||z.nodeType===9))if(M[1])return aa(z.getElementsByTagName(x),B);else if(M[2]&&D.find.CLASS&&z.getElementsByClassName)return aa(z.getElementsByClassName(M[2]),B);if(z.nodeType===9){if(x==="body"&&z.body)return aa([z.body],B);else if(M&&M[3]){var V=z.getElementById(M[3]);if(V&&V.parentNode){if(V.id===M[3])return aa([V],B)}else return aa([],B)}try{return aa(z.querySelectorAll(x),
B)}catch(o){}}else if(z.nodeType===1&&z.nodeName.toLowerCase()!=="object"){M=z;var A=(V=z.getAttribute("id"))||"__sizzle__",K=z.parentNode,F=/^\s*[+~]/.test(x);if(V)A=A.replace(/'/g,"\\$&");else z.setAttribute("id",A);if(F&&K)z=z.parentNode;try{if(!F||K)return aa(z.querySelectorAll("[id='"+A+"'] "+x),B)}catch(X){}finally{V||M.removeAttribute("id")}}}return i(x,z,B,J)};for(var u in i)y[u]=i[u];k=null}}();(function(){var i=L.documentElement,k=i.matchesSelector||i.mozMatchesSelector||i.webkitMatchesSelector||
i.msMatchesSelector;if(k){var u=!k.call(L.createElement("div"),"div"),x=false;try{k.call(L.documentElement,"[test!='']:sizzle")}catch(z){x=true}y.matchesSelector=function(B,J){J=J.replace(/\=\s*([^'"\]]*)\s*\]/g,"='$1']");if(!y.isXML(B))try{if(x||!D.match.PSEUDO.test(J)&&!/!=/.test(J)){var M=k.call(B,J);if(M||!u||B.document&&B.document.nodeType!==11)return M}}catch(V){}return y(J,null,null,[B]).length>0}}})();(function(){var i=L.createElement("div");i.innerHTML="<div class='test e'></div><div class='test'></div>";
if(!(!i.getElementsByClassName||i.getElementsByClassName("e").length===0)){i.lastChild.className="e";if(i.getElementsByClassName("e").length!==1){D.order.splice(1,0,"CLASS");D.find.CLASS=function(k,u,x){if(typeof u.getElementsByClassName!=="undefined"&&!x)return u.getElementsByClassName(k[1])};i=null}}})();y.contains=L.documentElement.contains?function(i,k){return i!==k&&(i.contains?i.contains(k):true)}:L.documentElement.compareDocumentPosition?function(i,k){return!!(i.compareDocumentPosition(k)&
16)}:function(){return false};y.isXML=function(i){return(i=(i?i.ownerDocument||i:0).documentElement)?i.nodeName!=="HTML":false};var ia=function(i,k,u){var x,z=[],B="";for(k=k.nodeType?[k]:k;x=D.match.PSEUDO.exec(i);){B+=x[0];i=i.replace(D.match.PSEUDO,"")}i=D.relative[i]?i+"*":i;x=0;for(var J=k.length;x<J;x++)y(i,k[x],z,u);return y.filter(B,z)};y.attr=c.attr;y.selectors.attrMap={};c.find=y;c.expr=y.selectors;c.expr[":"]=c.expr.filters;c.unique=y.uniqueSort;c.text=y.getText;c.isXMLDoc=y.isXML;c.contains=
...
"text html":true,"text json":c.parseJSON,"text xml":c.parseXML},flatOptions:{context:true,url:true}},ajaxPrefilter:Ea(Na),ajaxTransport:Ea(Ab),ajax:function(a,b){function d(i,k,u,x){if(ba!==2){ba=2;$&&clearTimeout($);S=j;H=x||"";T.readyState=i>0?4:0;var z,B,J;x=k;u=u?Oa(e,T,u):j;var M;if(i>=200&&i<300||i===304){if(e.ifModified){if(M=T.getResponseHeader("Last-Modified"))c.lastModified[t]=M;if(M=T.getResponseHeader("Etag"))c.etag[t]=M}if(i===304){x="notmodified";z=true}else try{B=Nb(e,u);x="success";
z=true}catch(V){x="parsererror";J=V}}else{J=x;if(!x||i){x="error";if(i<0)i=0}}T.status=i;T.statusText=""+(k||x);z?l.resolveWith(f,[B,x,T]):l.rejectWith(f,[T,x,J]);T.statusCode(n);n=j;if(aa)h.trigger("ajax"+(z?"Success":"Error"),[T,e,z?B:J]);m.fireWith(f,[T,x]);if(aa){h.trigger("ajaxComplete",[T,e]);--c.active||c.event.trigger("ajaxStop")}}}if(typeof a==="object"){b=a;a=j}b=b||{};var e=c.ajaxSetup({},b),f=e.context||e,h=f!==e&&(f.nodeType||f instanceof c)?c(f):c.event,l=c.Deferred(),m=c.Callbacks("once memory"),
...
jQuery.fn.extend({elementFrame:function(){var g=jQuery(this);if(arguments.length===1){var j=arguments[0],q=j.topleft,v=j.bottomright,p=q.x,s=j.width||v.x-q.x;v=j.height||v.y-q.y;g.css("top",q.y+"px");g.css("left",p+"px");if(!j.ignoreDimensions){g.width(s);g.height(v)}g.css("zIndex",j.zIndex);return this}else{g=jQuery(this);var r=g.offset();s=g.width();v=g.height();j=g.outerWidth()||s;q=g.outerHeight()||v;p=r.left;var w=p+j;r=r.top;var C=r+q;g=g.css("z-index");return{width:s,height:v,outerWidth:j,
outerHeight:q,zIndex:g,topleft:{x:p,y:r},topright:{x:w,y:r},bottomleft:{x:p,y:C},bottomright:{x:w,y:C}}}},windowFrame:function(){var g=jQuery(document.body),j=0,q=jQuery(document.body).width();if(self.innerHeight)j=self.innerHeight;else if(document.documentElement&&document.documentElement.clientHeight)j=document.documentElement.clientHeight;else if(document.body)j=document.body.clientHeight;var v=parseInt(g.css("margin-left"),10),p=parseInt(g.css("margin-right"),10);g=self.pageYOffset||jQuery.boxModel&&
...
q.topleft.x;g=q.topright.x;var s=q.bottomright.y,r=j.topleft.y,w=j.bottomleft.y,C=j.topleft.x,G=j.topright.x,I=j.width,N=j.height,O=r-v;if(O<0)O=0;var Q=s-w;if(Q<0)Q=0;var P=C-p;if(P<0)P=0;var R=g-G;if(R<0)R=0;j=this._availableFrameSizesHelper(q.width,O,v,g,r,p);var ea=this._availableFrameSizesHelper(q.width,Q,w,g,s,p),ta=this._availableFrameSizesHelper(P,q.height,v,G,s,p);q=this._availableFrameSizesHelper(R,q.height,v,g,s,G);var ua=this._availableFrameSizesHelper(P,O,v,C,r,p),va=this._availableFrameSizesHelper(I,
O,v,G,r,C);v=this._availableFrameSizesHelper(R,O,v,g,r,G);O=this._availableFrameSizesHelper(P,Q,w,C,s,p);I=this._availableFrameSizesHelper(I,Q,w,G,s,C);s=this._availableFrameSizesHelper(R,Q,w,g,s,G);p=this._availableFrameSizesHelper(P,N,r,C,w,p);g=this._availableFrameSizesHelper(R,N,r,g,w,G);return{top:j,bottom:ea,left:ta,right:q,topleft:ua,topcenter:va,topright:v,bottomleft:O,bottomcenter:I,bottomright:s,middleleft:p,middleright:g}},elementFramePercentHidden:function(){var g=jQuery(this),j=g.windowFrame(),
q=g.elementFrame();g=j.topleft.y;var v=j.topleft.x,p=j.bottomright.x,s=j.bottomright.y,r=q.topleft.y,w=q.bottomleft.y,C=q.topleft.x,G=q.topright.x,I=q.outerWidth;q=q.outerHeight;var N=rPixel=tPixel=bPixel=0,O=rPercent=tPercent=bPercent=0,Q=false;v=C-v;if(v<0){Q=true;N=v*-1;O=(I+v)/I*100}v=p-G;if(v<0){Q=true;rPixel=v*-1;rPercent=100-(I+v)/I*100}v=r-g;if(v<0){Q=true;tPixel=v*-1;tPercent=(q+v)/q*100}v=s-w+(!jQuery.browser.msie&&j.scrollWidth>j.width+16?-16:0);if(v<0){Q=true;bPixel=v*-1;bPercent=100-
...
jQuery(this).css("top",-1000).css("display","block"),v=jQuery(j.element),p=q.elementFrame(),s=v.elementFrame();q.windowFrame();for(var r=q.parents().get(),w=false,C=0;C<r.length;C++)if(jQuery(r[C]).css("position")=="relative"){w=true;break}if(w)C=r=0;else{r=s.topleft.x;C=s.topleft.y}j=this._getFrameOffsetPosition(v,s.height,s.width,j.vertical,j.hortizontal);r+=j.offsetX;C+=j.offsetY;v=this._getFrameOffsetPosition(q,p.height,p.width,g.vertical,g.hortizontal);j=r+(g.customOffsetX||0)-v.offsetX;v=C+
(g.customOffsetY||0)-v.offsetY;if(w){j-=r;v-=C}p={topleft:{x:j,y:v},width:p.width,height:p.height,ignoreDimensions:true};w=q.css("opacity");r=q.css("filter");C=q.css("-ms-filter");q.css("opacity",0).css("filter","alpha(opacity=0)").css("-ms-filter","progid:DXImageTransform.Microsoft.Alpha(Opacity=0)").elementFrame(p);if(!g.adjustFit||g.adjustFit.toLowerCase()==="autoadjust"||g.adjustFit.toLowerCase()==="slide")q.moveFrameIntoView();q.css("display","none").css("opacity",w).css("filter",r).css("-ms-filter",
C);return this},_getFrameOffsetPosition:function(g,j,q,v,p){function s(C,G){if(C.indexOf("%")>0){C=parseFloat(C)/100;return Math.floor(G*C)}else return C.length>0?parseInt(C):0}var r=0,w=0;switch(v.toLowerCase()){case "top":case "t":r=parseInt(g.css("margin-top"),10)||0;break;case "middle":case "m":case "center":case "c":r=Math.floor(j/2);break;case "bottom":case "b":r=j;break;default:r=s(v,j);if(isNaN(r))r=0}switch(p.toLowerCase()){case "left":case "l":w=parseInt(g.css("margin-left"),10)||0;break;
case "center":case "c":w=Math.floor(q/2);break;case "right":case "r":w=q;break;default:w=s(p,q);if(isNaN(w))w=0}return{offsetX:w,offsetY:r}},centerFrameInWindow:function(){var g=jQuery(this),j=false;if(g.is(":visible").length===0){j=true;g.css("top","-10000px").css("display","block")}var q=g.elementFrame(),v=g.windowFrame();j&&g.css("display","none");j=Math.floor((v.width-q.width)/2)+v.topleft.x;v=Math.floor((v.height-q.height)/2)+v.topleft.y;g.elementFrame({topleft:{x:j,y:v},width:q.width,height:q.height,
...
q){var v={sensitivity:7,interval:100,timeout:0};v=g.extend(v,q?{over:j,out:q}:j);var p,s,r,w,C=function(N){p=N.pageX;s=N.pageY},G=function(N,O){O.hoverIntent_t=clearTimeout(O.hoverIntent_t);if(Math.abs(r-p)+Math.abs(w-s)<v.sensitivity){g(O).unbind("mousemove",C);O.hoverIntent_s=1;return v.over.apply(O,[N])}else{r=p;w=s;O.hoverIntent_t=setTimeout(function(){G(N,O)},v.interval)}},I=function(N,O){O.hoverIntent_t=clearTimeout(O.hoverIntent_t);O.hoverIntent_s=0;return v.out.apply(O,[N])};j=function(N){for(var O=
(N.type=="mouseover"?N.fromElement:N.toElement)||N.relatedTarget;O&&O!=this;)try{O=O.parentNode}catch(Q){O=this}if(O==this)return false;var P=jQuery.extend({},N),R=this;if(R.hoverIntent_t)R.hoverIntent_t=clearTimeout(R.hoverIntent_t);if(N.type=="mouseover"){r=P.pageX;w=P.pageY;g(R).bind("mousemove",C);if(R.hoverIntent_s!=1)R.hoverIntent_t=setTimeout(function(){G(P,R)},v.interval)}else{g(R).unbind("mousemove",C);if(R.hoverIntent_s==1)R.hoverIntent_t=setTimeout(function(){I(P,R)},v.timeout)}};return this.mouseover(j).mouseout(j)}})(jQuery);
if(!this.JSON)this.JSON={};(function(){function g(G){return G<10?"0"+G:G}function j(G){p.lastIndex=0;return p.test(G)?'"'+G.replace(p,function(I){var N=w[I];return typeof N==="string"?N:"\\u"+("0000"+I.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+G+'"'}function q(G,I){var N,O,Q=s,P,R=I[G];if(R&&typeof R==="object"&&typeof R.toJSON==="function")R=R.toJSON(G);if(typeof C==="function")R=C.call(I,G,R);switch(typeof R){case "string":return j(R);case "number":return isFinite(R)?String(R):"null";case "boolean":case "null":return String(R);
...
10);if(!isNaN(s)&&s!==0)return s}p=p.parent()}}return 0},disableSelection:function(){return this.bind((g.support.selectstart?"selectstart":"mousedown")+".ui-disableSelection",function(p){p.preventDefault()})},enableSelection:function(){return this.unbind(".ui-disableSelection")}});g.each(["Width","Height"],function(p,s){function r(I,N,O,Q){g.each(w,function(){N-=parseFloat(g.curCSS(I,"padding"+this,true))||0;if(O)N-=parseFloat(g.curCSS(I,"border"+this+"Width",true))||0;if(Q)N-=parseFloat(g.curCSS(I,
"margin"+this,true))||0});return N}var w=s==="Width"?["Left","Right"]:["Top","Bottom"],C=s.toLowerCase(),G={innerWidth:g.fn.innerWidth,innerHeight:g.fn.innerHeight,outerWidth:g.fn.outerWidth,outerHeight:g.fn.outerHeight};g.fn["inner"+s]=function(I){if(I===j)return G["inner"+s].call(this);return this.each(function(){g(this).css(C,r(this,I)+"px")})};g.fn["outer"+s]=function(I,N){if(typeof I!=="number")return G["outer"+s].call(this,I);return this.each(function(){g(this).css(C,r(this,I,true,N)+"px")})}});
...
{plugin:{add:function(p,s,r){p=g.ui[p].prototype;for(var w in r){p.plugins[w]=p.plugins[w]||[];p.plugins[w].push([s,r[w]])}},call:function(p,s,r){if((s=p.plugins[s])&&p.element[0].parentNode)for(var w=0;w<s.length;w++)p.options[s[w][0]]&&s[w][1].apply(p.element,r)}},contains:function(p,s){return document.compareDocumentPosition?p.compareDocumentPosition(s)&16:p!==s&&p.contains(s)},hasScroll:function(p,s){if(g(p).css("overflow")==="hidden")return false;s=s&&s==="left"?"scrollLeft":"scrollTop";var r=
false;if(p[s]>0)return true;p[s]=1;r=p[s]>0;p[s]=0;return r},isOverAxis:function(p,s,r){return p>s&&p<s+r},isOver:function(p,s,r,w,C,G){return g.ui.isOverAxis(p,r,C)&&g.ui.isOverAxis(s,w,G)}})}})(jQuery);(function(g,j){if(g.cleanData){var q=g.cleanData;g.cleanData=function(p){for(var s=0,r;(r=p[s])!=null;s++)try{g(r).triggerHandler("remove")}catch(w){}q(p)}}else{var v=g.fn.remove;g.fn.remove=function(p,s){return this.each(function(){if(!s)if(!p||g.filter(p,[this]).length)g("*",this).add([this]).each(function(){try{g(this).triggerHandler("remove")}catch(r){}});
return v.call(g(this),p,s)})}}g.widget=function(p,s,r){var w=p.split(".")[0],C;p=p.split(".")[1];C=w+"-"+p;if(!r){r=s;s=g.Widget}g.expr[":"][C]=function(G){return!!g.data(G,p)};g[w]=g[w]||{};g[w][p]=function(G,I){arguments.length&&this._createWidget(G,I)};s=new s;s.options=g.extend(true,{},s.options);g[w][p].prototype=g.extend(true,s,{namespace:w,widgetName:p,widgetEventPrefix:g[w][p].prototype.widgetEventPrefix||p,widgetBaseClass:C},r);g.widget.bridge(p,g[w][p])};g.widget.bridge=function(p,s){g.fn[p]=
function(r){var w=typeof r==="string",C=Array.prototype.slice.call(arguments,1),G=this;r=!w&&C.length?g.extend.apply(null,[true,r].concat(C)):r;if(w&&r.charAt(0)==="_")return G;w?this.each(function(){var I=g.data(this,p),N=I&&g.isFunction(I[r])?I[r].apply(I,C):I;if(N!==I&&N!==j){G=N;return false}}):this.each(function(){var I=g.data(this,p);I?I.option(r||{})._init():g.data(this,p,new s(r,this))});return G}};g.Widget=function(p,s){arguments.length&&this._createWidget(p,s)};g.Widget.prototype={widgetName:"widget",
...
_mouseDrag:function(){},_mouseStop:function(){},_mouseCapture:function(){return true}})})(jQuery);(function(g){g.ui=g.ui||{};var j=/left|center|right/,q=/top|center|bottom/,v={},p=g.fn.position,s=g.fn.offset;g.fn.position=function(r){if(!r||!r.of)return p.apply(this,arguments);r=g.extend({},r);var w=g(r.of),C=w[0],G=(r.collision||"flip").split(" "),I=r.offset?r.offset.split(" "):[0,0],N,O,Q;if(C.nodeType===9){N=w.width();O=w.height();Q={top:0,left:0}}else if(C.setTimeout){N=w.width();O=w.height();
Q={top:w.scrollTop(),left:w.scrollLeft()}}else if(C.preventDefault){r.at="left top";N=O=0;Q={top:r.of.pageY,left:r.of.pageX}}else{N=w.outerWidth();O=w.outerHeight();Q=w.offset()}g.each(["my","at"],function(){var P=(r[this]||"").split(" ");if(P.length===1)P=j.test(P[0])?P.concat(["center"]):q.test(P[0])?["center"].concat(P):["center","center"];P[0]=j.test(P[0])?P[0]:"center";P[1]=q.test(P[1])?P[1]:"center";r[this]=P});if(G.length===1)G[1]=G[0];I[0]=parseInt(I[0],10)||0;if(I.length===1)I[1]=I[0];I[1]=
...
{using:r.using}))})};g.ui.position={fit:{left:function(r,w){var C=g(window);C=w.collisionPosition.left+w.collisionWidth-C.width()-C.scrollLeft();r.left=C>0?r.left-C:Math.max(r.left-w.collisionPosition.left,r.left)},top:function(r,w){var C=g(window);C=w.collisionPosition.top+w.collisionHeight-C.height()-C.scrollTop();r.top=C>0?r.top-C:Math.max(r.top-w.collisionPosition.top,r.top)}},flip:{left:function(r,w){if(w.at[0]!=="center"){var C=g(window);C=w.collisionPosition.left+w.collisionWidth-C.width()-
C.scrollLeft();var G=w.my[0]==="left"?-w.elemWidth:w.my[0]==="right"?w.elemWidth:0,I=w.at[0]==="left"?w.targetWidth:-w.targetWidth,N=-2*w.offset[0];r.left+=w.collisionPosition.left<0?G+I+N:C>0?G+I+N:0}},top:function(r,w){if(w.at[1]!=="center"){var C=g(window);C=w.collisionPosition.top+w.collisionHeight-C.height()-C.scrollTop();var G=w.my[1]==="top"?-w.elemHeight:w.my[1]==="bottom"?w.elemHeight:0,I=w.at[1]==="top"?w.targetHeight:-w.targetHeight,N=-2*w.offset[1];r.top+=w.collisionPosition.top<0?G+I+
N:C>0?G+I+N:0}}}};if(!g.offset.setOffset){g.offset.setOffset=function(r,w){if(/static/.test(g.curCSS(r,"position")))r.style.position="relative";var C=g(r),G=C.offset(),I=parseInt(g.curCSS(r,"top",true),10)||0,N=parseInt(g.curCSS(r,"left",true),10)||0;G={top:w.top-G.top+I,left:w.left-G.left+N};"using"in w?w.using.call(r,G):C.css(G)};g.fn.offset=function(r){var w=this[0];if(!w||!w.ownerDocument)return null;if(r)return this.each(function(){g.offset.setOffset(this,r)});return s.call(this)}}(function(){var r=
document.getElementsByTagName("body")[0],w=document.createElement("div"),C,G;C=document.createElement(r?"div":"body");G={visibility:"hidden",width:0,height:0,border:0,margin:0,background:"none"};r&&jQuery.extend(G,{position:"absolute",left:"-1000px",top:"-1000px"});for(var I in G)C.style[I]=G[I];C.appendChild(w);G=r||document.documentElement;G.insertBefore(C,G.firstChild);w.style.cssText="position: absolute; left: 10.7432222px; top: 10.432325px; height: 30px; width: 201px;";w=g(w).offset(function(N,
O){return O}).offset();C.innerHTML="";G.removeChild(C);r=w.top+w.left+(r?2E3:0);v.fractions=r>21&&r<22})()})(jQuery);if(typeof jQuery.ui.jig==="undefined")jQuery.ui.jig={};jQuery.ui.jig._decodeJSON=function(g){try{return eval("({"+g+"})")}catch(j){jQuery.ui.jig._isConsole("error")&&console.error("Error parsing jig metatag, or jig-dataconfig. Problem with JSON string: '"+g+"'. Error: "+j.message+".")}};jQuery.ui.jig._setLocalConfig=function(g,j){if(g=g.attr("config")){g=jQuery.ui.jig._decodeJSON(g);
...
function(){var g=jQuery(document.body),j=g.height(),q=g.data("_lastHeight");if(q!==j){q&&g.trigger("ncbijigpageheightchanged");g.data("_lastHeight",j)}};jQuery.ui.jig.requiresLoginURL="/account/signin/?inlinelogin=true";jQuery.ui.jig.requiresLoginWidth=440;jQuery.ui.jig.requiresLoginHeight=540;jQuery.ui.jig.embeddedIframe=null;jQuery.ui.jig._requiresLoginIframe=null;jQuery.ui.jig.requiresLogin=function(g){function j(){g&&g(Ka(),false)}function q(){if(!jQuery.ui.jig.embeddedIframe){var v=jQuery.ui.jig.requiresLoginWidth,
p=jQuery.ui.jig.requiresLoginHeight,s=jQuery('<div style="display:none"><a href="#jig-ncbi_requires_login">Inline Login Component</a></div><div id="jig-ncbi_requires_login" title="My NCBI Sign In"><iframe style="width:100%;height:100%;" src="'+jQuery.ui.jig.requiresLoginURL+'"></iframe></div>').css("display","none").appendTo(document.body,true);s.find("a").attr("href","#jig-ncbi_requires_login").ncbidialog({width:v,height:p}).ncbidialog("open");jQuery.ui.jig._requiresLoginIframe=s;jQuery.ui.jig.waitForLoginCookie(function(r){var w=
r||Ka()||" ";try{jQuery("#myncbiusername").show();jQuery("#mnu").html(w);jQuery("#sign_in").hide();jQuery("#sign_out, #myncbi").show();s.find("a:eq(0)").ncbidialog("close");document.body.removeChild(s.get(0))}catch(C){}g&&g(r,true,window.location.href)})}}jQuery.ui.jig._requiresLoginIframe&&jQuery.ui.jig._requiresLoginIframe.hide().remove();jQuery.ui.jig.checkIfLoggedIn(j,q)};jQuery.ui.jig.getElementWidgets=function(g,j){g=jQuery(g).data();var q=[];for(var v in g)g[v]instanceof jQuery.ui[j]&&q.push(g[v]);
return q};jQuery.ui.jig._isLoggedIn=null;jQuery.ui.jig.checkIfLoggedIn=function(g,j){var q=jQuery.ui.jig._isLoggedIn;Ka()===null?j():window.setTimeout(function(){q===null?jQuery.ajax({url:"https://"+window.location.hostname+(window.location.port?":"+window.location.port:"")+"/account/ajax/issignedin/?callback=?",dataType:"jsonp",jsonpCallback:"issignedin",cache:false,success:function(v){v=v&&v.is_signed_in;jQuery.ui.jig._isLoggedIn=v;jQuery.ui.jig._checkIfLoggedInNotify(v,g,j)},error:function(){jQuery.ui.jig._isLoggedIn=
null;jQuery.ui.jig._checkIfLoggedInNotify(false,g,j)}}):jQuery.ui.jig._checkIfLoggedInNotify(q,g,j)},50)};jQuery.ui.jig._autoCheckState=function(){function g(){jQuery.ui.jig.checkIfLoggedIn(jQuery.ui.jig._autoCheckState,jQuery.ui.jig._autoCheckState)}jQuery.ui.jig._autoCheckStateTimer&&window.clearTimeout(jQuery.ui.jig._autoCheckStateTimer);jQuery.ui.jig._isLoggedIn=null;jQuery.ui.jig._autoCheckStateTimer=window.setTimeout(g,6E5)};jQuery.ui.jig._checkIfLoggedInNotify=function(g,j,q){if(g&&j)j();else!g&&
q&&q();g||jQuery.ui.jig._removeLoggedInFlag()};jQuery.ui.jig.waitForLoginCookie_logincheck=false;jQuery.ui.jig.waitForLoginCookie=function(g){var j=function(){var q=jQuery.cookie("WebCubbyUser");q&&q.indexOf("logged-in=true")>-1?window.setTimeout(g,30):window.setTimeout(j,10)};if(jQuery.ui.jig.waitForLoginCookie_logincheck&&jQuery.ui.jig._isLoggedIn===null)jQuery.ui.jig._isLoggedIn_wait(j);else{jQuery.ui.jig.waitForLoginCookie_logincheck=true;window.setTimeout(function(){jQuery.ui.jig.checkIfLoggedIn(j,
j)},50)}};jQuery.ui.jig._isLoggedIn_wait=function(g){jQuery.ui.jig._isLoggedIn!==null?g():window.setTimeout(function(){jQuery.ui.jig._isLoggedIn_wait(g)},10)};jQuery.ui.jig._removeLoggedInFlag=function(){var g=jQuery.cookie("WebCubbyUser");if(g&&g.indexOf("logged-in=true")>-1){var j=jQuery.cookie("lout");if(!(j&&new Date-parseInt(j,10)<1E4)){g=g.replace("logged-in=true","logged-in=false");jQuery.cookie("WebCubbyUser",g,{path:"/",domain:".nih.gov"});jQuery.cookie("lout",(new Date).getTime())}}};(function(){jQuery(window).load(function(){jQuery.ui.jig.hasOnloadFired=
true});jQuery.ui.jig.hasOnloadFired=false})();var ja=Z.ui;if(typeof ja.jig==="undefined")ja.jig={};var Y=ja.jig;document.location.search.search(/jig_debug=true/i)!==-1&&typeof window.console==="undefined"&&Z(function(){Z("body").prepend('<div style="border: 2px solid #ccc; height: 150px; overflow:auto;"><h2 style="color:red">jig Console</h2><ul id="jig-cons"></ul></div>');console={};for(var g=["log","info","warn","group","groupEnd"],j=function(p){Z("#jig-cons").append("<li>"+p+"</li>")},q=0;q<g.length;q++){var v=
g[q];if(typeof console[v]==="undefined")console[v]=j}});document.compatMode==="BackCompat"&&Y._isConsole("warn")&&console.warn("Document is in quirks mode. jig widgets only work properly when in standards mode. Please add or correct your DOCTYPE definition in your page.");Y.scanned=false;Y.version="1.12";Y._jsExt=null;Y._cssExt=null;Y._foundWidgets={};Y._widgDict={ncbiaccordion:new W({name:"ncbiaccordion",selector:"div.jig-ncbiaccordion, div.jig-accordion",overrideDefaults:{autoHeight:false,header:":header"}}),
ncbibutton:new W({name:"ncbibutton",selector:".jig-ncbibutton, .jig-ncbibutton-gray, .jig-ncbibutton-blue, .jig-ncbibutton-copper",interactions:["button"]}),ncbibuttonset:new W({name:"ncbibuttonset",selector:".jig-ncbibuttonset"}),ncbibasicmenu:new W({name:"ncbibasicmenu",selector:".jig-ncbibasicmenu"}),ncbimenu:new W({name:"ncbimenu",selector:"ul.jig-ncbimenu",dependsOn:["ncbibasicmenu"]}),ncbilocalnav:new W({name:"ncbilocalnav",selector:"ul.jig-ncbilocalnav",dependsOn:["ncbimenu"]}),ncbidatepicker:new W({name:"ncbidatepicker",
selector:"input.jig-ncbidatepicker"}),ncbielastictextarea:new W({name:"ncbielastictextarea",selector:"textarea.jig-ncbielastictextarea"}),ncbidialog:new W({name:"ncbidialog",selector:"a.jig-ncbidialog, button.jig-ncbidialog, input.jig-ncbidialog, textarea.jig-ncbidialog",interactions:["draggable","resizable"],overrideDefaults:{autoOpen:false}}),ncbiautocomplete:new W({name:"ncbiautocomplete",selector:'input[type="text"].jig-ncbiautocomplete,input[type="search"].jig-ncbiautocomplete'}),ncbigrid:new W({name:"ncbigrid",
selector:"div.jig-ncbigrid table,div.jig-ncbigrid-scroll table,table.jig-ncbigrid,table.jig-ncbigrid-scroll",dependsOn:["ncbipopper"]}),ncbiservergrid:new W({name:"ncbiservergrid",selector:"div.jig-ncbiservergrid table,div.jig-ncbiservergrid-scroll table,table.jig-ncbiservergrid,table.jig-ncbiservergrid-scroll",dependsOn:["ncbigrid"]}),ncbilinkedselects:new W({name:"ncbilinkedselects",selector:"select.jig-ncbilinkedselects"}),ncbipopper:new W({name:"ncbipopper",selector:".jig-ncbipopper",dependsOn:["ncbibutton"]}),
ncbislideshow:new W({name:"ncbislideshow",selector:"div.jig-ncbislideshow"}),ncbitoggler:new W({name:"ncbitoggler",selector:"a.jig-ncbitoggler,a.jig-ncbitoggler-open,a.ui-ncbitoggler,a.ui-ncbitoggler-open"}),ncbitree_base:new W({name:"ncbitree_base",selector:"ul.jig-ncbitree_base"}),ncbitree:new W({name:"ncbitree",selector:"ul.jig-ncbitree",dependsOn:["ncbitree_base"]}),ncbitabs:new W({name:"ncbitabs",selector:"div.jig-ncbitabs, div.jig-tabs, "}),ncbilinksmenu:new W({name:"ncbilinksmenu",selector:".jig-ncbilinksmenu",
dependsOn:["ncbipopper"]}),ncbiclearbutton:new W({name:"ncbiclearbutton",selector:".jig-ncbiclearbutton"}),ncbiplaceholder:new W({name:"ncbiplaceholder",selector:"input[placeholder], textarea[placeholder]"}),ncbiexpander:new W({name:"ncbiexpander",selector:".jig-ncbiexpander"}),ncbiselect:new W({name:"ncbiselect",selector:".jig-ncbiselect"}),ncbiinpagenav:new W({name:"ncbiinpagenav",selector:".jig-ncbiinpagenav"}),ncbismoothscroll:new W({name:"ncbismoothscroll",selector:".jig-ncbismoothscroll"}),
ncbispinbox:new W({name:"ncbispinbox",selector:'[type="number"]'}),ncbiformvalidator:new W({name:"ncbiformvalidator",selector:"form"}),ncbihelppopper:new W({name:"ncbihelppopper",selector:".jig-ncbihelppopper"}),ncbihelpwindow:new W({name:"ncbihelpwindow",selector:"a.jig-ncbihelpwindow"}),ncbiautofocus:new W({name:"ncbiautofocus",selector:".jig-ncbiautofocus"}),ncbihistogram:new W({name:"ncbihistogram",selector:".jig-ncbihistogram"}),ncbisetswitch:new W({name:"ncbisetswitch",selector:".jig-ncbisetswitch",
dependsOn:["ncbilinksmenu"]}),ncbiexternallink:new W({name:"ncbiexternallink",selector:"body",dependsOn:["ncbipopper"]}),ncbiactivevalidation:new W({name:"ncbiactivevalidation",selector:".jig-ncbiactivevalidation"})};var qa=Y._widgDict,Ca=[];for(var ka in qa)Ca.push(ka);var Fc=["draggable","resizable","sortable"];Y.scanjig=function(g,j){var q,v;if(j){v=j.widgets||Ca;q=j.configs||{}}else{q={};v=Ca}for(var p=0;p<v.length;p++){g=g||Z(document.body);var s=v[p];if(!(Ja&&Z.inArray(s,Xa)===-1)){var r=ja[s];
if(typeof r==="undefined"&&!Y.scanned)Y._isConsole("warn")&&console.warn('jig: widget "'+s+'" does not exist. Check documentation (http://iwebdev/core/jig/ for correct name');else{!Y.scanned&&typeof r._customLocalConfig==="undefined"&&function(I,N,O){I.prototype._create=function(){var Q;(Q=this.element.attr("config"))&&Z.ui.jig._isConsole("warn")&&console.warn('The use of the "config" attribute to set options on '+O+' is deprecated. Use "data-jigconfig" instead.');Q||(Q=this.element.data("jigconfig"));
if(Q){Q=typeof Q==="string"?ja.jig._decodeJSON(Q):Q;Z.extend(this.options,Q)}if(O==="droppable"){jQuery(this.options.dragEls);var P=qa.droppable.callbacks[this.options.dropCallback];typeof P!=="undefined"&&this._setData("drop",function(){P()})}if((Q=qa[O].interactions||null)&&Q.length>0){Q="";if(this.options.draggable&&!ja.draggable)Q+='<script type="text/javascript" src="/core/ui/'+ja.version+'/development-bundle/ui/jquery.ui.draggable.js"><\/script>';if(this.options.resizable&&!ja.resizable)Q+=
'<script type="text/javascript" src="/core/ui/'+ja.version+'/development-bundle/ui/jquery.ui.resizable.js"><\/script>';Z("head").append(Q)}N.apply(this,[]);Y._foundWidgets[O]=(Y._foundWidgets[O]||0)+1}}(r,r.prototype._create,s);(function(I){r.getSelector=function(){return qa[I].selector}})(s);j={};for(var w in q)if(w===s)j=q[w];var C=Gc(Hc(s))||{};if(typeof C.contextNode!=="undefined")g=Z(C.contextNode);var G=jQuery.extend(C,j);j=qa[s].overrideDefaults;typeof j!=="undefined"&&Z.extend(G,j);j=ja[s].getSelector();
C=Z(g);j=C.is(j)?C:C.find(j);j.length>0&&j.each(function(){var I=jQuery(this);if(!I.data("jigscanned"+s)){I.data("jigscanned"+s,true);I[s](G);Ic(r,I)}})}}}Y.scanned=true};var Ic=function(g,j){var q=g.prototype.options.accessibilityFallback;if(g.prototype.options.accessibilityFallback){var v=g.prototype.widgetName,p=typeof q==="function"?q(j):j,s=j.attr("id")?j.attr("id"):Z.ui.jig._generateId(v);j.attr("id",s);var r=(g=g.prototype.options.accessibilityFallbackCustomDestroy)?g:function(){jQuery("#"+
s)[v]("destroy")};g=v.substr(4);j=Z.ui.jig._generateId(v);jQuery('<p class="ui-helper-hidden-accessible">The following '+g+' user interface control may not be accessible. Tab to the next button to revert the control to an accessible version.</p><a id="'+j+'" class="ui-helper-hidden-accessible" class="jig-destroy" role="button" href="#" title="Click to revert the following '+g+' user interface control to an accessible version">Destroy user interface control</a>').insertBefore(p);jQuery("#"+j).click(function(w){w.preventDefault();
r(p)})}};Y.scan=Y.scanjig;var ha=["after","before","append","html","prepend"];for(ka=0;ka<ha.length;ka++){var Da=ha[ka];origMethod=Z.fn[Da];(function(g,j){Z.fn[j]=function(q,v){q=g.apply(this,[q]);if(v)j!=="after"&&j!=="before"?Z.ui.jig.scan(q,v):Z.ui.jig.scan(q.parent(),v);return q}})(origMethod,Da)}var Jc=/(.*\/)?js\/jig(?:\.nojquery)?(\.min)?\.js$/;Z("script").each(function(){var g=Z(this);if(g=Z.trim(g.attr("src")))if(g=g.match(Jc)){Y._baseURL=g[1]||"";if(g[2]===".min"){Y._isMin=true;Y._jsExt=
".min.js";Y._cssExt=".min.css"}else{Y._isMin=false;Y._jsExt=".js";Y._cssExt=".css"}return false}});var pa=Y._baseURL;pa=Y._baseURL;var Ma=Y._jsExt,La=Y._cssExt,Hc=function(g){var j=Ya.length;if(j>0)for(var q=0;q<j;q++){var v=Ya[q],p=Z.trim(v.getAttribute("name"));if(g===p)return v}},Gc=function(g){g=Z(g);var j=g.attr("content");if(j){g.attr("name");return ja.jig._decodeJSON(j)}},Kc=function(g){document.write('<link type="text/css" rel="stylesheet" href="'+pa+"css/jquery.ui."+g+La+'"/>')},Db=function(g){document.write('<script type="text/javascript" src="'+
